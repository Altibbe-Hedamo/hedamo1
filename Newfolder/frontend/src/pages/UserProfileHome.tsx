    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          if (refreshProfileStatus) await refreshProfileStatus();
        } else {
      ...
      {loading ? (
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="flex items-center mb-8">
            ...
            />
            <div>
              <h2 className="text-2xl font-semibold text-gray-800">{profile.full_name || profile.name || 'User Name'}</h2>
              <p className="text-gray-600">{profile.email_address || profile.email || 'Email not specified'}</p>
            </div>
          </div>
          <Section title="Personal Identification & Contact Details" icon="👤">
            <Field label="Full Name" value={profile.full_name || profile.name || 'Not specified'} />
            <Field label="Date of Birth" value={formatDate(profile.date_of_birth || profile.dob)} />
            <Field label="Gender" value={profile.gender || 'Not specified'} />
            <Field label="Mobile Number" value={profile.mobile_number || profile.phone || 'Not specified'} />
            <Field label="Email Address" value={profile.email_address || profile.email || 'Not specified'} />
            <Field label="Current Address" value={profile.current_address || profile.address || 'Not specified'} />
            <Field label="Permanent Address" value={profile.permanent_address || 'Not specified'} />
          </Section>
          <Section title="Identity Verification & Financial Details" icon="🔍">
            <Field label="ID Number" value={profile.id_number || profile.govIdNumber || 'Not specified'} />
